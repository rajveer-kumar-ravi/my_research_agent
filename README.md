...
The agent runs a bounded, multi-step research loop, backed by a robust repository and database infrastructure:
...
3. **Scrape & clean** — pages are fetched safely (SSRF-guarded via robust URL and scheme validation blocking internal IPs, localhosts, and cloud metadata endpoints), stripped of boilerplate/ads/scripts, and chunked.
...
| **Scraping & Security** | httpx + BeautifulSoup4 + custom validators | Async-capable HTTP, robust HTML parsing, and strict SSRF host/scheme blocking |
...
| **Database & Persistence** | SQLite / PostgreSQL + SQLAlchemy | Zero-setup local persistence with runtime zero-downtime schema patching (`_ensure_columns_exist`) and repository encapsulation |
...
│   │   ├── core/                  # config.py (env settings), logging.py (scoped loggers)
│   │   ├── models/                # SQLAlchemy ORM (research.py, user.py, session.py)
...
│   │   ├── db/                    # database.py (engine, get_db, db_session, schema patching), repository.py
│   │   └── utils/                 # text_cleaner.py, validators.py (SSRF guards, truncation)
...
| **`DATABASE_URL`** | No (default local SQLite file) | Automatically handles cloud providers by translating `postgres://` to `postgresql://` |
| **`REDIS_URL`** | No | Optional Redis connection string for caching/workers with graceful fallback (`None` on failure) |
...
| **`GET`** | `/api/research` | Paginated history (`?limit=&offset=`) via repository abstraction |
| **`DELETE`** | `/api/research/{id}` | Delete a research record via repository abstraction |
...
12. Limitations & Enterprise Enhancements
...
SQLite / Postgres Persistence: Enhanced with runtime schema patching (_ensure_columns_exist) and encapsulated via the Repository Pattern (ResearchRepository), keeping data access decoupled from application business logic.

Security & Lifecycle: Hardened against SSRF vulnerabilities via strict scheme and host pattern checks, supported by secure FastAPI generator dependencies (get_db) and context-managed background workers (db_session).
